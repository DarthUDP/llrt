...
uint32_t crc32_impl_lookup(const uint8_t *data, size_t data_len, const uint32_t *table, int mode, uint32_t orig)
{
	size_t i;
...
uint32_t crc32_calc(const uint8_t *data, size_t data_len)
{
	...
	calc = crc32_impl_lookup(data, data_len, crc32_table, 0, 0);
	return calc;
}
...
bool crc32_check(const uint8_t *data, size_t data_len, uint32_t orig)
{
	...
	gen_table(crc32_table, CRC32POLY, true, true);
	calc = crc32_impl_lookup(data, data_len, crc32_table, 1, orig);
	return calc;
}
